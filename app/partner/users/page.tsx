...
export default function PartnerUsersPage() {
  const { data, isLoading } = useSWR<{ users: ReferredUser[]; total: number }>(
    "/api/partner/users",
    fetcher
  );
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted text-sm">
            {data?.total || 0} مستخدم تم إحالتهم من رابطك
          </p>
        </div>
      ...
      <div className="bg-surface-1 border border-card-border rounded-2xl overflow-hidden">
        {filteredUsers.length === 0 ? (
          <div className="p-12 text-center text-muted">
            ...
            <p className="text-sm">
              {search ? "لا توجد نتائج مطابقة" : "لا يوجد مستخدمون مُحالون بعد"}
            </p>
          </div>
        ) : (
              ...
              <tbody>
                {filteredUsers.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <PlanBadge
                        plan={user.billing?.plan_key || "none"}
                      />
                    </td>
